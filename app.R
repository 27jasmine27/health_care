...
page_one <- tabPanel(
  ...
  sidebarLayout(
    sidebarPanel(
      radioButtons(
        inputId = "age", label = h3("Age Range"),
        choices = list(
          "Ages 0-24",
          "Ages 25-34",
          "Ages 35-44",
      ...
      "*Non-colored states have no sufficient data for the specified age group"
    ),
    mainPanel(
      plotlyOutput("overdose_map"),
      "We can see from the map that the areas with the most overdose
      deaths were concentrated on the eastern portion of the US. We can
      also see that the state with the most overdoses for each age group
      except 55+ was Ohio. The state with the most for the 55+ age group was
      California."
    )
  )
)
...
page_four <- tabPanel(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      "We can see that for all of the US states, there has been a pretty
      steady increase in the amount of deaths over the years."
    )
  )
)
...
page_two <- tabPanel(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      "Looking at the graphs, we can see that the age group with the most
      overdose deaths (at least for 2017 and 2015) was the 25-34 age group
      and the one with the least is the 0-24 age group."
    )
  )
)
...
server <- function(input, output) {
  ...
  observe({
    if (input$category != "") {
      ...
      proxy %>% addPolylines(
        stroke = TRUE, weight = 4, color = "red",
